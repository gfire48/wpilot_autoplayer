...
OptionParser.prototype = {
    ...
    parse: function(args) {
        ...
        while((token = tokens.shift()) && this._halt == false) {
            // Support the conventional --option=value form as well as
            // --option value.  Previously the complete token was compared
            // with the rule name, so a value supplied after '=' was silently
            // ignored.
            if(LONG_SWITCH_RE.exec(token)) {
                var equals_index = token.indexOf('=');
                if(equals_index != -1) {
                    tokens.unshift(token.substr(equals_index + 1));
                    token = token.substr(0, equals_index);
                }
            }
